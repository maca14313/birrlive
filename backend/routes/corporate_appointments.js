...
router.post('/create/corporate', async (req, res) => {

 
console.log('corporate')
const { aTitle, aDesc, appointmentCreator, pendingParticipants } = req.body;


console.log(aTitle, aDesc, appointmentCreator, pendingParticipants)
try {
  const [result] = await pool.query(
    `INSERT INTO Appointments (Title, Description,AppointmentCreator, PendingParticipants,AppointmentsType) 
     VALUES (?, ?, ?, ?,?)`,
    [aTitle, aDesc, appointmentCreator, JSON.stringify(pendingParticipants),'corporate']
  );
  ...
  const appointmentId = result.insertId;
    ...
    const [appointmentCreatorResult] = await pool.query(
      `SELECT userOrgContacts FROM SignUp WHERE phoneNumber = ?`,
      [appointmentCreator]
    );

    if (appointmentCreatorResult.length > 0) {
      let userOrgContacts = [];
      const userContactsRaw = appointmentCreatorResult[0].userOrgContacts;

      if (typeof userContactsRaw === 'string' && userContactsRaw.trim() !== '') {
        try {
          userOrgContacts = JSON.parse(userContactsRaw);
        } catch (e) {
          console.error('Error parsing userOrgContacts:', e);
          userOrgContacts = [];
        }
      }

      const newContacts = pendingParticipants.filter(phoneNumber => !userOrgContacts.includes(phoneNumber)); // Filter new contacts

      if (newContacts.length > 0) {
        const updatedContacts = Array.from(new Set([...userOrgContacts, ...newContacts])); // Ensure unique contacts
...
router.post('/create/personal', async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      `INSERT INTO Appointments (Title, Description, ADate, ATime, RDate, RTime, ALocation, AppointmentCreator, PendingParticipants, AppointmentsType,CreatedBy) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)`,
      [aTitle, aDesc, aDate, aTime24, rDate, rTime24, aLocation, appointmentCreator, JSON.stringify(pendingParticipants), 'personal','corporate']
    );
...
// Route to get corporate appointments for a specific user
router.post('/user-corporate-p-appointments', async (req, res) => {
  ...
  try {
    ...
    const [appointments] = await pool.query(
      `SELECT * FROM Appointments WHERE id IN (?) AND AppointmentsType = 'corporate'`,
      [pAppointments]
    );
...
// Route to get corporate personal appointments for a specific user
router.post('/user-corporate-personal-p-appointments', async (req, res) => {
  const { phoneNumber } = req.body;
  ...
  try {
    ...
    // Get the personal appointment details from the Appointments table
    const [appointments] = await pool.query(
      `SELECT * FROM Appointments WHERE id IN (?) AND AppointmentsType = 'personal' AND CreatedBy='corporate' `,
      [pAppointments]
    );
    ...
    console.log(appointments,';iutresdfghjuycdfcvghuyrtdesxdcfvghytrcfvbhngcfvbhjygftgvbhgygvbg')
    res.status(200).json({ appointments });
  } catch (error) {
    console.error('Error retrieving user appointments:', error);
    res.status(500).json({ message: 'Failed to retrieve user appointments' });
...
router.post('/from-pending-participants-to-participants', async (req, res) => {
  const { appointmentId, phoneNumber,aDesc, aDate, aTime, aLocation } = req.body;
  console.log('Received request:', appointmentId, phoneNumber,aDesc, aDate, aTime, aLocation);
  ...
  try {
    ...
    await pool.query(
      `UPDATE Appointments 
       SET PendingParticipants = ?, Participants = ?, Description = ?, ADate = ?, ATime = ?, ALocation = ?
       WHERE id = ?`,
      [JSON.stringify(PendingParticipants), JSON.stringify(Participants),aDesc,aDate, aTime24, aLocation ,appointmentId]
    );
    ...
    res.status(200).json({ message: 'Participants and user appointments updated successfully' });
  } catch (error) {
    console.error('Error updating participants and user appointments:', error);
    res.status(500).json({ message: 'Failed to update participants and user appointments' });
  }
});
...
router.post('/fetch-creator-corporate-appointments', async (req, res) => {
  const { phoneNumber } = req.body;

  try {
    // Get the appointments where the phoneNumber is the AppointmentCreator
    const [appointments] = await pool.query(
      `SELECT * FROM Appointments WHERE AppointmentCreator = ? AND AppointmentsType = 'corporate' AND PendingParticipants IS NOT NULL AND JSON_LENGTH(PendingParticipants) > 0 `,
      [phoneNumber]
    );
...
// Route to get corporate appointments for a specific user using Appointments field
